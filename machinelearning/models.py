...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        """
        param = self.param
        l1 = nn.ReLU(nn.AddBias(nn.Linear(x, param[0]), param[1]))
        l2 = nn.AddBias(nn.Linear(l1, param[2]), param[3])
        return l2
    ...
    def train(self, dataset):
        ...
        while loss > 0.018:
            for x, y in dataset.iterate_once(batch_size):
                ...
                gradient = nn.gradients(loss, self.param)
                for i in range(len(gradient)):
                    self.param[i].update(gradient[i], rate)
...
class DigitClassificationModel(object):
    ...
    def train(self, dataset):
        ...
        #while acc < 0.99:
            #for x,y in dataset.iterate_once(batch_size):
                #gradient = nn.gradients(self.get_loss(x, y), self.param)
